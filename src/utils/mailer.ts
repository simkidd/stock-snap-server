...
export class MailService {
  ...
  async sendMail(payload: MailOptions): Promise<boolean> {
    const toEmail =
      typeof payload.to === 'string' ? payload.to : payload.to.address;
    const toName = typeof payload.to === 'string' ? '' : payload.to.name;

    if (!config.BREVO_API_KEY) {
      console.log(
        `[Brevo Email Mock] To: ${toEmail} | Subject: "${payload.subject}" (Set BREVO_API_KEY in .env to deliver real emails)`,
      );
      return true;
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log(
          `[Brevo Email] Message sent successfully to ${toEmail}. MessageId: ${resJson.messageId}`,
        );
        return true;
      } else {
        const errText = await response.text();
        console.error(
          `[Brevo Email Error] Status ${response.status}: ${errText}`,
        );
        return false;
      }
    } catch (err) {
  ...
  async sendSms(options: SmsOptions): Promise<boolean> {
    ...
    if (!config.BREVO_API_KEY) {
      console.log(
        `[Brevo SMS Mock] To: ${formattedRecipient} | Sender: ${sender} | Message: "${options.content}"`,
      );
      return true;
    }
    ...
    try {
      const response = await fetch(
        'https://api.brevo.com/v3/transactionalSMS/send-transacSms',
        {
          method: 'POST',
          headers: {
            accept: 'application/json',
      ...
      if (response.ok) {
        const resJson = await response.json();
        console.log(
          `[Brevo SMS] SMS delivered to ${formattedRecipient}. MessageId: ${resJson.messageId}`,
        );
        return true;
      } else {
        const errText = await response.text();
        console.error(
          `[Brevo SMS Error] Status ${response.status}: ${errText}`,
        );
        return false;
      }
    } catch (error) {
